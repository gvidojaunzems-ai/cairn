# Cairn Build Spec 20 · Security, privacy & consent (cross-cutting)

**Build order:** Hardening (audits all earlier work; fixing issues it finds is in scope). Verify the whole folder.
...
## 1. Goal
Make Cairn's privacy promises real and enforced — codify the invariants, harden secrets, control outbound network, enforce consent, and add a test suite that **fails the build** if an invariant is violated.

## 2. Requirements
### 2.1 Privacy invariants (enforced + tested)
1. **No code in WIP signals** — parse emitted `signals/*.json`; fail on any code/diff content.
2. **Meeting audio stays on-device** — assert no audio file remains after a session and no audio egress.
3. **Secrets never in repo/DB/logs** — scan the team repo, `cairn.db`, `local.config.json`, and logs for secret patterns; keys/tokens only in the OS keychain.
4. **Claude only on explicit/route + metered** — assert local-tier tasks make no external call; every Claude call is ledgered + attributable.
5. **Team repo holds no secrets/PII beyond intended shared state.**
### 2.2 Outbound network allowlist
Only permitted egress: the local model runtime (localhost), the Claude API (on route), the GitHub API (bootstrap/PR data), and configured RSS/feed/Slack endpoints. Everything else blocked/asserted. A test captures attempted egress and fails on anything off-list. No analytics/telemetry except the opt-in channel (off by default).
### 2.3 Secret & data hardening
Keychain-backed secrets with a documented encrypted-file fallback; **log redaction** for token-like/PII strings; consider at-rest protection for `cairn.db`/cache (record the decision as an ADR); clear-secrets-on-sign-out/uninstall guidance.
### 2.4 Consent & permissions
Meeting recording is opt-in per session with a visible indicator + explicit stop; AI **proposals never auto-apply** (re-audit the Projects + Meeting features); map which Core API ops can cause egress/external writes and ensure each is gated by a user action/route; least-privilege GitHub token scopes (read-only where possible; document required scopes).
### 2.5 Supply chain
Dependency vulnerability scan in CI; pin/lock dependencies; review native/binary sidecars (whisper.cpp) provenance; document an update policy.
### 2.6 Threat model
Publish `/docs/security/threat-model.md` (assets, trust boundaries, risks, mitigations, residual risks); keep it updated.

## 3. Acceptance criteria
- The invariant test suite passes and is wired into CI as a **release gate** (a violation fails the build).
- The egress test shows only allowlisted endpoints across a full app exercise.
- The secret scan over repo/DB/logs is clean; logs are redacted.
- Consent is required to record; disabling signal emission is honored; proposals require explicit apply.
- The dependency scan runs in CI; the threat model is published.

## 4. Handoff / Risks
This task may require fixing earlier work — in scope. Test invariants at the seams, not just in prose.

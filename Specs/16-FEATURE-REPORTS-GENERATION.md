# Cairn Build Spec 16 · Feature: Reports & document generation

**Build order:** Feature (needs UI shell + AI engine; writes via git-sync engine). **UI reference: `Cairn-Prototype.html#reports`.**
...
## 1. Goal
Turn real activity into polished documents in one click, with an explicit **local (free) vs Claude (polished)** engine choice and export. Reproduce `#reports`.

## 2. Requirements
- **Templates:** Daily Standup, Weekly Status, PoC Summary, **PoC Retro/Postmortem**, Monthly PoC one-pager, **Release Notes** (from branch commits). Each declares its grounding scope.
- **Grounding:** assemble inputs from approved standups, git history/WIP, charter + guardrail state, decisions, tickets — for the chosen scope (person/PoC/week). Pass to `ai.complete{taskType:'report.<kind>'}`; display the **grounded refs** ("grounded in N commits, M PRs…").
- **Engine toggle:** **Fast & free** (local, 0 tokens) vs **Polished** (Claude). Show a **token estimate** before a Claude run (`ai.estimate`); route accordingly (external=true → Claude when polished), enforce the budget, update the meter. Default internal/daily docs to local.
- **Preview & edit:** render the draft; allow inline edits before export/publish.
- **Export:** Markdown, **DOCX**, **PDF** (valid, openable files saved to a user-chosen path). **Push to repo:** write to `pulse/` or the relevant project/meeting file via the git engine.
- **Ops used:** `reports.templates/generate(type,engine,scope)/export(id,format)/pushToRepo(id)`. Subscribe to `budget.updated`.

## 3. Acceptance criteria
- Each template generates grounded output citing real activity from fixtures.
- Engine toggle: local run spends 0 budget; polished run routes to Claude, shows an estimate first, and updates the meter; budget-exceeded falls back to local with a clear flag.
- Export produces valid .md/.docx/.pdf; push writes the document into the team repo (verify).
- Editing the preview is reflected in exports/pushes.

## 4. Handoff / Risks
Test DOCX/PDF validity per OS. Keep generation cancelable and backgrounded for large scopes.

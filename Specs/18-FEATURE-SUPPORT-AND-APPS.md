# Cairn Build Spec 18 · Feature: Support & Apps

**Build order:** Feature (needs UI shell + git-sync engine + collectors). **UI reference: `Cairn-Prototype.html#support`.**
...
## 1. Goal
A lightweight home for the background apps and their occasional tickets — kept separate so it never clutters PoC work — so nothing gets dropped. Reproduce `#support`.

## 2. Requirements
- **Apps:** list `apps` with health (ok/degraded/down), owner, last deploy, optional error rate. A connected monitoring feed (via the collectors/settings) surfaces degradations here; without it, it's a manual but central registry.
- **On-call:** show the weekly on-call person (from `cairn.config.yaml`/apps); simple rotation.
- **Ticket inbox:** list `tickets` with title, app, **age**, **priority**, assignee, status. Tickets arrive from configured sources (email/Slack/form via the collectors) or manual entry.
- **Triage/resolve:** assign (e.g., to on-call) via `support.triageTicket`; **resolve** (`support.resolveTicket`) writes a `## Resolution` to the ticket (via the git engine) and **saves the fix as a knowledge note** (indexed by search).
- **Draft changelog:** generate a changelog from recent fixes via `ai.complete{report.releasenotes}` / the reports engine.
- **Ops used:** `support.listApps/getApp/listTickets/triageTicket/resolveTicket`. Subscribe to `sync.updated`.

## 3. Acceptance criteria
- Apps + tickets render from fixtures with health/age/priority; on-call shown.
- Triage assigns; resolve writes the resolution to the ticket file and creates a searchable knowledge note (verify round-trip).
- A connected mock monitoring feed flips an app to degraded.
- Stays visually/logically separate from the PoC screens.

## 4. Handoff / Risks
Test from fixtures + mock connectors. Keep it low-friction for the twice-a-month reality — no heavyweight workflow.

# Cairn Build Spec 17 · Feature: Team Pulse

**Build order:** Feature (needs UI shell + git-sync engine; AI for the weekly digest). **UI reference: `Cairn-Prototype.html#pulse`.**
...
## 1. Goal
A whole-squad view assembled automatically from git-synced markers — replacing status meetings — plus an optional weekly digest. Reproduce `#pulse`.

## 2. Requirements
- **Stats strip:** active PoCs, unpushed >3d, drifting off-goal, status freshness.
- **Project-health heatmap:** PoC × recent activity, color-coded (healthy/idle/stalled), derived from git timestamps + signals; updates on `sync.updated`.
- **Who's on what:** per teammate, current PoC + status from latest `updates`/`signals`; show **unpushed-day flags** and drift where relevant.
- **Blockers & recently shipped:** aggregated lists across the squad.
- **Weekly digest:** `pulse.generateWeeklyDigest` synthesizes the week (one **Claude** call, budgeted — an allowed polished use); can be **pushed to `pulse/`** and/or produced by a scheduled CI job (document the CI option).
- **Ops used:** `pulse.get/generateWeeklyDigest`. Subscribe to `sync.updated`.

## 3. Acceptance criteria
- Heatmap + who's-on-what + blockers + shipped render from fixtures; freshness reads "today" when markers are current.
- Unpushed/drift flags appear where fixtures warrant.
- The weekly digest routes one polished call (budget updates) and can be pushed to `pulse/`.
- No manual data entry — everything aggregated.

## 4. Handoff / Risks
Test from fixtures. Present as aggregation of work, not individual monitoring (copy/info bubbles per the prototype).

# Cairn Build Spec 14 · Feature: News & Knowledge

**Build order:** Feature (needs UI shell + collectors + search index). **UI reference: `Cairn-Prototype.html#news`.**
...
## 1. Goal
Stay current without doom-scrolling: a locally-summarized AI-news feed connected to the squad's work, plus a searchable knowledge base of saved items and decisions. Reproduce `#news`.

## 2. Requirements
- **Feed:** list `news_items` (produced by the collectors) with title, source, age, 2-line summary, tags, and a per-PoC **"why it matters"**. Filter by source chips (Anthropic, OpenAI, Hugging Face, arXiv, Hacker News, …) and by topic cluster. Convey that summaries are local / 0-token.
- **Actions:** **Save to knowledge** (`news.save`) persists a knowledge note (a `decisions/`-style entry via the git engine, indexed by search); **"Why it matters"** can (re)generate a grounded note via `ai.complete{news.why}` (local); Open original (external).
- **Company / Slack highlights:** optional section if the Slack connector is configured; the local feed works fully without it.
- **Knowledge base:** list saved items + decisions; **semantic search** via `search.query` across knowledge (optionally all indexed content); deep-link results to their source.
- **History:** surface the backfilled history/topics — browse by topic cluster, not just "today".
- **Ops used:** `news.listFeed/getItem/save/listKnowledge`, `search.query`. Subscribe to `news.updated`.

## 3. Acceptance criteria
- The feed shows items with summaries/tags/why-it-matters; filters by source and topic work; clearly 0-token/local.
- Save writes a knowledge note to the team repo and it becomes searchable — verify round-trip.
- Knowledge search returns relevant saved items/decisions with working deep links.
- With history backfilled, the user can browse past topics, not only the latest day.

## 4. Handoff / Risks
Test against a fixture feed + index. Rank by relevance to active PoCs; keep "why it matters" concise.

# Cairn Build Spec 13 · Feature: Meeting listener (fully local)

**Build order:** Feature (needs UI shell + AI engine; writes via the git-sync engine). **UI reference: `Cairn-Prototype.html#meetings`.**
...
## 1. Goal
Capture decisions and actions from a meeting **entirely on-device** (local speech-to-text), then **propose** updates to action items, project status, and even the charter/vision for the user to approve — **never auto-applied**. Reproduce `#meetings`.

## 2. Requirements
- **Consent & indicator:** recording is **opt-in per session** with a clearly visible "listening" indicator; explicit Start/Stop.
- **Local STT:** invoke the bundled whisper.cpp (or equivalent) sidecar; stream a partial transcript to the UI via the `meeting.partial` event. **Audio stays on device and is discarded after transcription** — never written beyond a transient buffer, never uploaded (privacy invariant; add a test).
- **Extraction:** on stop (or incrementally), run `ai.complete{taskType:'meeting.extract'}` (structured, local) → a summary, **decisions**, **action items with owners**, **risks/blockers**, and **proposed updates** of types: tick a completed action item; change a PoC status; mark a blocker resolved; **charter/deadline/vision change**.
- **Proposal review:** list proposals, each with Accept / Edit / Dismiss. **Charter/vision and deadline changes are gated** — require explicit approval and are visually distinguished. "Accept all & save note". **Nothing applies until accepted.**
- **Apply:** accepted proposals write via the git engine (update project status/charter, action items, resolved blockers) and persist the **meeting note** to `meetings/<date>-<slug>.md` (summary + decisions + owner-tagged action items; no audio). Emits `sync.updated`.
- Extraction runs locally by default; an optional "polished summary" may route to Claude (budgeted) if the user chooses.
- **Ops used:** `meetings.start/stop/getLive/getProposals/applyProposal/applyAll/get`; events `meeting.partial`, `meeting.proposals`.

## 3. Acceptance criteria
- Recording requires explicit start; the indicator is visible while active; stop discards audio (assert no audio file persists and nothing is uploaded — privacy test).
- Transcription produces text locally from a sample audio fixture; extraction yields decisions + owner-tagged action items + proposals.
- Applying a proposal writes the correct change to the team repo; a charter/vision change cannot apply without explicit approval; declining changes nothing.
- Works offline for record→extract with local models.

## 4. Handoff / Risks
Bundle whisper.cpp per-OS (coordinate with the packaging task). STT accuracy/latency: chunk audio, show partials, allow manual edit of transcript/proposals before applying.

# Cairn Build Spec 11 · Feature: PoC Projects, charter & guardrails

**Build order:** Feature (needs UI shell + git-sync engine + AI engine + search index). **UI reference: `Cairn-Prototype.html#projects`.**
...
## 1. Goal
Track PoCs with **derived status**, a **charter**, and **goal/deadline guardrails**, plus the **describe-it → search-our-data → draft-charter → create-and-track** onboarding flow. Reproduce `#projects`.

## 2. Requirements
- **List/cards:** filter by status; each card shows status badge, goal, **deadline countdown**, **on-goal %** alignment bar, next step, owner, last activity. Status auto-inferred from git activity (active<3d/idle 3–7d/stalled>7d/shipped) with a human override that wins.
- **Detail view:** **Charter** (goal, success_criteria[], non_goals[], deadline, suggested owner — editable; saved via `projects.updateCharter` → writes `projects/<slug>.md`); a **timeline** auto-built from git (commits/PRs/branches/decisions); an **AI summary** (`ai.complete{poc.summary}`, local); a **decision log** (linked `decisions/`); actions **Generate status report** and **Generate retro** (`projects.generateRetro`, via the reports engine).
- **Guardrails:** **goal-drift** via `ai.complete{drift.check}` comparing recent activity to the charter → on-goal/adjacent/off-goal score + a **drift flag with a human-readable reason** (advisory; never auto-changes the charter); surfaced on cards, detail, the Today focus strip, and Dailies. **Deadline burndown**: elapsed vs deadline, risk flag when progress lags.
- **New-PoC onboarding (modal):** free-text description → "Search our data & draft the PoC": run `search.query` over docs/repos/past PoCs/tickets and show **what was found** (reusable assets) → draft a **charter** via `ai.complete{charter.infer}` grounded in the findings → **Create & start tracking** via `projects.create(fromDescription)` (writes the project + charter through the git engine, begins status/guardrail tracking). Mirror the prototype flow. Document any repo scaffolding created at kickoff.
- **Ops used:** `projects.list/get/create/updateCharter/setStatus/archive/generateRetro`, `search.query`, reports ops. Subscribe to `sync.updated`.

## 3. Acceptance criteria
- Status + on-goal% + deadline derive correctly from fixtures; manual override persists.
- A drifting fixture PoC (e.g., `poc-agent-router` doing a config rewrite) is flagged off-goal with a reason.
- New-PoC flow shows found assets (rag-eval harness, retrieval doc, tickets), drafts a charter, and creates a tracked PoC (appears in the list, written to the team repo).
- Generate status report + retro produce grounded output; editing the charter writes back to `projects/<slug>.md`.

## 4. Handoff / Risks
Test charter editing + guardrails on fixtures. Keep drift advisory (always show the reason; never auto-edit the charter/vision).

# Cairn Build Spec 15 · Feature: Docs Hub (structured wiki)

**Build order:** Feature (needs UI shell + git-sync engine + search index; AI for drafts). **UI reference: `Cairn-Prototype.html#docs`.**
...
## 1. Goal
A full, polished documentation product that stands on its own: structured wiki navigation, search, "Ask the docs", a clean reading view, plus **auto-import from repos** and **AI-drafting** — all backed by Markdown in the team repo (`docs/`). Reproduce `#docs`.

## 2. Requirements
- **Structure & nav:** collapsible groups — **Handbook, Architecture, Runbooks, Decisions (ADRs), PoC archive, From repositories** (auto). Each item shows a **status dot** (ok / auto-imported / AI-draft / stale). Active highlight; keyboard-navigable.
- **Toolbar:** full-text/semantic **search** that filters the tree and shows results; an **Ask the docs** panel returning a grounded, **cited** answer via `search.askDocs` (local, 0-token); **Sync repos** (`docs.syncRepos`); a doc-stats line (pages, auto-imported, drafts-to-review, coverage).
- **Reading view (center + right rail):** breadcrumb; title; **metadata** (owner/avatar, last-updated, source badge, tags); rich Markdown (headings, lists, code, callouts, tables); a **related-docs** footer; a right-rail **on-this-page TOC** + page actions (Edit, History).
- **Create/edit:** **New doc** from a template (runbook, ADR, design doc, retro) or **AI draft** via `ai.complete{doc.draft}` from code/commits; an editor with edit→preview→**commit** that writes Markdown to `docs/**` via the git engine (versioned by git).
- **Auto-import & drafts:** import READMEs/`/docs` from repos (kept in sync; flagged "auto-imported"); **auto-draft** pages where coverage is missing (flagged "AI draft — review"); surface **stale** pages for refresh.
- Docs are plain Markdown in the team repo — editable in the user's own editor too; the hub renders and manages them.
- **Ops used:** `docs.tree/get/create/save/syncRepos/listDrafts`, `search.askDocs/query`. Subscribe to `sync.updated`.

## 3. Acceptance criteria
- Structured nav renders with groups/status dots; collapsing + selecting pages works; narrow widths collapse the TOC (responsive).
- Search filters the tree/shows results; **Ask the docs** returns a cited answer with working deep-links, 0-token.
- Creating/editing a doc commits Markdown to `docs/**` (verify in the team repo) and it appears in nav + becomes searchable.
- Auto-imported READMEs, an AI-draft (flagged), and a stale page appear as in the prototype; "Sync repos" updates imported pages.
- The reading view shows breadcrumb/metadata/TOC/related correctly.

## 4. Handoff / Risks
This must be genuinely useful standalone (an explicit product requirement) — invest in the reading view, search quality, and structure. Verify the Markdown round-trip to the repo.

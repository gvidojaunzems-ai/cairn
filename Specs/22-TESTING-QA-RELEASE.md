# Cairn Build Spec 22 · Testing, QA & release

**Build order:** Hardening (continuous; gates release). Feature tasks contribute their own tests; you own the cross-cutting suites + gates + release. Verify the folder.
...
## 1. Goal
Guarantee quality across the whole build: a layered test strategy, contract + privacy + performance gates, an accessibility pass, and a repeatable release process traceable back to each spec's acceptance criteria.

## 2. Requirements
### 2.1 Test layers
- **Unit:** pure logic (status derivation, routing/budget math, parsers, charter/guardrails).
- **Integration:** engines against real seams — a **temp git repo** (team-repo round-trips), **mock Ollama** + **mock Claude** (routing/budget), **mock GitHub** + **mock feeds** (collectors), a **fixture index** (search). Provide these doubles as shared test utilities.
- **Contract tests:** assert the five stable contracts (team-repo file schema, local store schema, Core API ops/events, AI task contract, domain model) so independently-built chunks stay compatible.
- **End-to-end:** drive the packaged app through key flows — first-run bootstrap → populated app; approve standup → repo write; new-PoC onboarding; meeting record→extract→apply; docs create/edit/ask; report generate/export; a settings change → behavior change.
- **Privacy/invariant suite** and **performance suite** run as **release gates**.
- **Accessibility:** automated checks (roles/contrast/focus) + a keyboard-only pass per screen.
### 2.2 Fixtures & determinism
Maintain the shared fixture dataset + deterministic AI doubles so tests are stable/offline. No test depends on real network or real model output.
### 2.3 CI gates
Every change: lint + typecheck + unit + integration + contract + privacy on **Windows (required)** + a second OS. E2E + perf + packaging on tagged releases. Coverage targets enforced. A red gate blocks merge/handoff.
### 2.4 QA & release
A **manual QA checklist** per release (cross-OS smoke; the key flows; offline behavior; missing-Ollama behavior; budget-exceeded behavior; consent/recording; conflict handling). A **release process:** version bump + CHANGELOG; build/sign/notarize; run all gates; publish artifacts + update feed; tag; a documented, rehearsed **rollback**. An **acceptance-traceability matrix** mapping each spec's Acceptance Criteria → the automated test(s) covering them; track gaps.

## 3. Acceptance criteria
- All layers run in CI with correct gating; the suite is green and deterministic/offline.
- Contract, privacy, and performance suites are wired as release gates and demonstrably fail on a seeded violation.
- A documented **release dry-run** produces signed artifacts and passes every gate; rollback rehearsed.
- The traceability matrix covers every spec's acceptance criteria; `/docs/qa/` holds the checklist, matrix, and release runbook.

## 4. Handoff / Risks
Continuous WP: each feature contributes tests; you own the cross-cutting suites + gates + release. Enforce the deterministic-doubles rule to avoid flaky AI/network tests.

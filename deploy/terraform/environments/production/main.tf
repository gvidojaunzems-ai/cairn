...
module "runtime_sa" {
  ...
  display_name = "Cairn runtime (${var.env})"
  roles        = []
}
...
# guarantees a globally-unique name. force_destroy=false so a stray `terraform
# destroy` cannot wipe published installers.
...
output "runtime_sa_email" {
  description = "Runtime service account email (grant additional resource-scoped bindings against this)."
